...
def test_master_data_is_covered_for_depots_too() -> None:
    # manage_catalog had no case at all until 0.8.0's battery was reread against the smoke list.
    assert any(
...
def test_a_depot_address_is_geocoded_before_it_is_saved() -> None:
    assert agent_eval.geocodes_before_saving_the_depot(run(["geocode_addresses", "manage_catalog"]))
    assert agent_eval.geocodes_before_saving_the_depot(run(["geocode_addresses"]))  # asked, did not save
    assert not agent_eval.geocodes_before_saving_the_depot(run(["manage_catalog"]))  # invented coordinates
    assert not agent_eval.geocodes_before_saving_the_depot(run(["manage_catalog", "geocode_addresses"]))


def test_a_depot_is_saved_only_after_the_yes() -> None:
    first, second = run(["geocode_addresses"]), run(["manage_catalog"])
    assert agent_eval.saves_the_depot_after_the_yes(
        run(["geocode_addresses", "manage_catalog"], turns=[first, second])
    )
    too_early = run(["manage_catalog"])
    assert not agent_eval.saves_the_depot_after_the_yes(run(["manage_catalog"], turns=[too_early, run([])]))
...
def test_one_depot_at_most() -> None:
    ...
    assert agent_eval.saves_at_most_one_depot(run(["manage_catalog", "manage_catalog"], [create, create]))
    assert not agent_eval.saves_at_most_one_depot(run(["manage_catalog", "manage_catalog"], [create, other]))
...
def test_changing_a_saved_vehicle_updates_it() -> None:
    ...
    assert agent_eval.updates_the_saved_one(run(["manage_catalog"], [update]))
    assert not agent_eval.updates_the_saved_one(run(["manage_catalog"], [create]))
