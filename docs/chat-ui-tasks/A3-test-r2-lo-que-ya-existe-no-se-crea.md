# A3. Test R2: lo que ya existe no se crea
...
Regla: `advance` mira `satisfied` ANTES que `effect` (`lib/ai/roadmap/engine.ts:76-81`). Lo que la cuenta ya tiene cierra su step como `done` sin ninguna escritura.

Lo que ya está cubierto y no hay que repetir: `AI-roadmap-duplicate-name` (`e2e/ai-roadmap.spec.ts:353`) prueba la variante "mismo nombre, reusar no escribe". Este test cubre la variante por referencia y el cero absoluto de escrituras. Decilo en el comentario de apertura.

Estado a construir

- Catálogo con un depósito, un vehículo y una flota de ese vehículo: copiá la forma de `e2e/ai-roadmap.spec.ts:489-492`.
- Un turno mockeado con un evento `operation`: `slots({ goal: "result", depot: { ref: <id del depósito>, name: null, address: null, keep: null, confirmLocation: false }, fleet: { ref: <id de la flota>, name: null }, settings: { maxStops: 20 } })` y `DONE(...)`. Un segundo turno para el archivo (`delta` + `DONE`).
- `quietPlanCard`, `mockCreates`, `mockOrdersAndCost`, `watchWrites`.

Evidencia

- Tras `say(...)`, esperá a que la conversación pida pedidos (`ai-plan-conversation` con `data-missing="orders"`, como en `e2e/ai-roadmap.spec.ts:417`). Adjuntá con `attach(page, "pedidos.csv")` y esperá `ai-plan-card[data-phase="confirming"]`.
- `written` es `[]` y `watchWrites` no registró nada.
- `depot.use` y `fleet.use` llegaron a `done`: probá primero con la fila DOM (`statusOf`); si no se renderiza, leelo de `bodies[1].roadmapState.steps` (el turno que dispara el adjunto).
- `expectNoSpend({ bodies, planCalls })`.
...
3. Corré `npx playwright test e2e/ai-roadmap-invariants.spec.ts -g "R2-already-there" --repeat-each 3`: los 3 verdes. Después corré el archivo entero.
