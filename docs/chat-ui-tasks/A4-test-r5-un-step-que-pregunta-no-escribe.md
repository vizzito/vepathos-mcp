# A4. Test R5: un step que pregunta no escribe
...
Regla: dentro de un step listo la pregunta va antes que el efecto (`lib/ai/roadmap/engine.ts:92-108`). Un step que pregunta no escribe en la misma pasada.

Trampa: la regla es POR STEP, no global. La doc dice "si la página pregunta, no se escribió nada", pero `AI-roadmap-keep-everything` (`e2e/ai-roadmap.spec.ts:204-217`) prueba lo contrario: otros steps escriben mientras la ubicación es pregunta. Por eso este test arma un roadmap con UN solo escritor. Decilo en el comentario de apertura.

Estado a construir

- Catálogo con un vehículo existente llamado `QA Camioneta 800` (forma en `e2e/ai-roadmap.spec.ts:354`).
- `operation` con `slots({ goal: "resources", keepVehicles: true, vehicles: [{ name: "QA Camioneta 800", count: null, weightKg: 800, volumeM3: 6 }] })`.
- `mockCreates`, `watchWrites`, `quietPlanCard`.

Evidencia

1. Esperá `ai-roadmap-question` con `data-question="vehicle.create:duplicate"`.
2. `const n = written.length` (tiene que ser 0). `await page.waitForTimeout(3000)`, que es más que las retenciones del ejecutor. Afirmá que `written.length === n` y que `watchWrites` sigue vacío.
3. `const answeredAt = Date.now()`. Click en `ai-roadmap-option-create_another`. Esperá `ai-saved-vehicle`.
4. Afirmá `written.length === n + 1`, `written[n].what === "vehicle"` y `written[n].at >= answeredAt`: la escritura existe y es posterior a la respuesta.
5. Si `create_another` abre otra pregunta (por ejemplo pedir un nombre nuevo), contestala de la forma más simple y anotá en el handoff qué pidió.
...
3. Corré `npx playwright test e2e/ai-roadmap-invariants.spec.ts -g "R5-ask-before-write" --repeat-each 3`: los 3 verdes. Después corré el archivo entero.
