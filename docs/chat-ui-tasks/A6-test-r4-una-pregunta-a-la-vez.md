# A6. Test R4: una pregunta a la vez
...
Regla: `advance` devuelve UNA jugada (`RoadmapMove`, `lib/ai/roadmap/types.ts:285-289`; `ask ??=` en `engine.ts:89,95` y la vuelta en `:128`). La página renderiza una sola pregunta (`pendingQuestion`).

Dos correcciones a la doc, que van en el comentario de apertura:

- Varios steps pueden tener `data-status="asking"` a la vez (`engine.ts:92-96`): lo que es único es la pregunta RENDERIZADA. Afirmá `ai-roadmap-question`, nunca cuántos steps están `asking`.
- La mitad "a lo sumo un `ai-ask` sin responder" NO es un invariante de la página: con varias preguntas del modelo abiertas, el turno sale cuando están todas respondidas (`components/ai/use-ai-chat.ts`, `answerAsk`, cerca de `:799-811`). No la afirmes.

Lo que ya está cubierto: `AI-roadmap-one-off-policy` (`e2e/ai-roadmap.spec.ts:307-314`) afirma en UN instante que la política va antes que la ubicación. Este test afirma la regla en TODO el recorrido, con muestreo continuo.

Estado a construir: el de `e2e/ai-roadmap.spec.ts:288-296` (política con `plan.noRecord: true` y `preferences.askBeforeAlternatives: true`, depósito con `confirmLocation: true` y `keep: false`, vehículos on demand con `keepVehicles: false`). No hay escritores.

Evidencia

1. `const read = await watchDom(page)` ANTES de `say(...)`.
2. Recorré: esperá `policy:alternatives`, click `ai-roadmap-option-accept`; esperá `depot.locate:confirm`, click `ai-roadmap-option-candidate:0`; esperá que el roadmap pida pedidos (`ai-roadmap-doors`).
3. `const seen = await read()`: `seen.maxQuestions === 1`, y `seen.questions` contiene `policy:alternatives` antes que `depot.locate:confirm`.
4. `written` vacío.

Opcional y fuera de la aceptación (solo si sobra presupuesto): un noveno test `R4-asks: several model questions go back in one turn, once all are answered`, con un turno mockeado que emite dos eventos `ask` con `callId` distintos (forma en `lib/ai/events.ts:230-242`, ejemplo real en `e2e/ai-planner.spec.ts:118-228`). Evidencia: tras responder el primero `bodies.length` no cambió; tras responder el segundo hay exactamente un body nuevo cuyo `toolAnswers` trae los dos `callId`.
...
3. Corré `npx playwright test e2e/ai-roadmap-invariants.spec.ts -g "R4-one-question" --repeat-each 3`: los 3 verdes. Después corré el archivo entero.
