# B8. Corrida sin gasto, comparación y cierre
...
## Tarea B8: corrida sin gasto, comparación y cierre

Pasos

1. Pedile a Martín que confirme en el prompt y corré `npx tsx e2e-live/run.ts --label luna --runs 3` (solo los casos que no gastan).
2. En `/Users/martinvizzolini/workspace/vepathos-mcp`: `python3 devtools/agent_eval.py --compare` (no gasta nada). La columna `luna/<modelo>` tiene que aparecer junto a las de MCP (Claude Code) y ChatGPT.
3. Revisión final del arnés:
   - Sin `--spends`, el ledger de TODA la corrida no tiene ningún `optimize`, ningún approval aprobado y ningún POST a `/api/routehub/`.
   - Ningún check de prosa en `passed`. Lo no medido sale `-`, nunca `0/N`.
   - El banner imprimió modelo, cuenta y costo estimado antes de empezar, y pidió confirmación.
   - Ningún valor de clave en ninguna salida ni en el JSON.
4. Handoff de cierre:
   - La tabla propia y la de `--compare`, pegadas.
   - Dónde la web se aparta del MCP con el mismo modelo (filas donde la columna web difiere de `luna.gpt-5.6-luna` de `openai_eval`).
   - Lo que NO se mide acá y dónde se mide (doc §6): `NO confirma fechas como teléfono`, `no nombra tools al usuario`, `responde en castellano`, `sin shell ni subagentes`, "otro intento, nunca gratis".
   - La línea de npm script que proponés para que Martín la agregue a mano si quiere: `"test:ai-live": "npx tsx e2e-live/run.ts"`. No edites `package.json`.
   - `git status --short -- e2e e2e-live`, y confirmación de cero commits: `git log -1 --format=%H` igual que en `A0.md`, misma rama.
