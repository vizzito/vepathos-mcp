# A8. Test R6: cost se niega a cerrar
...
Regla: `cost.satisfied` (`lib/ai/roadmap/actions.ts:593-601`) devuelve `null`, o sea NO cierra, cuando el plan está estacionado en un ajuste de flota para la revisión actual (`parkedOnAdjust`, `:30-32`) o cuando `plan.stops > plan.maxStops * plan.units` y la persona no eligió seguir parcial para esa revisión. Y `phase === "starting"` explícitamente NO cuenta como arrancado (`:599-600`). Como `run` va después de `cost`, en ninguno de esos casos puede empezar.

Lo que ya está cubierto: "estacionado en un ajuste, no se pide el costo" vive en `AI-adjust-fleet` (`e2e/ai-roadmap.spec.ts:512-517`). Dato para el comentario de apertura: los tests unitarios no cubren `starting` (la palabra no aparece en `lib/ai/__tests__/roadmap-engine.test.ts`), este test es el único lugar.

Estado a construir: el de `e2e/ai-roadmap.spec.ts:489-498` (depósito, flota de 2 unidades con `max_stops` 20, `settings: { maxStops: 1 }`, 3 pedidos: 3 > 1 * 2). Pero la ruta de `/api/ai/plan` la escribís vos en vez de usar `mockOrdersAndCost`, porque necesitás retener el `optimize`:

- `preflight`: `{ preflight: preflightFor(body, 3) }`.
- `optimize`: `await gate` (una promesa que el test resuelve) y después `route.fulfill({ status: 422, json: { error: "test_refused", code: "TEST_REFUSED" } })`. Usá 422, no 409.
- Mockeá también `**/api/ai/upload` (como en `e2e/ai-roadmap.spec.ts:102`) y `POST **/api/optimization/plans` (copiá `e2e/ai-roadmap.spec.ts:409-414`), porque `runPlan` guarda los pedidos antes de optimizar (`components/ai/use-ai-chat.ts:1812-1863`).

Tramos y evidencia

a) Con `ai-roadmap-question[data-question="cost:infeasible"]` abierta: `cost` no está `done`, `run` no empezó, `planCalls` vacío (ni siquiera un preflight), `expectNoSpend`.
b) Click `ai-roadmap-option-continue`: llega al menos un `preflight`, `ai-plan-card` llega a `data-phase="confirming"`, y `cost` SIGUE sin cerrar: con goal `result`, `confirming` no es arrancado.
c) `starting`: click en el `ai-plan-run` con scope dentro de `ai-plan-review`. Afirmá `ai-plan-card[data-phase="starting"]` y, con el optimize retenido, que `run` no está `running` y `ai-plan-launched` no existe. Soltá el `gate`. La tarjeta vuelve a `data-phase="editing"` con `ai-plan-error[data-code]` (mirá `use-ai-chat.ts:1831-1835`), `ai-plan-launched` sigue ausente, `cost` no está `done`, y en `planCalls` hay exactamente UN `optimize`: el intento rechazado.

Si el estado de `cost` no se puede leer durante `starting` (la fila no se renderiza y no hay turno de chat que traiga `roadmapState`), afirmá las consecuencias observables (tramo c) y decí en el handoff qué parte quedó sin observar directamente.
...
3. Corré `npx playwright test e2e/ai-roadmap-invariants.spec.ts -g "R6-cost-refuses" --repeat-each 3`: los 3 verdes. Después corré el archivo entero.
