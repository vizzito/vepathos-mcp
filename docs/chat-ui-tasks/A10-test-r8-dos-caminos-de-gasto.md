# A10. Test R8: dos caminos de gasto
...
## Tarea A10: test de R8

Regla: hay DOS caminos de gasto, no uno. El ticket de aprobación no es la única forma de gastar paradas: `ai-plan-run` (`components/ai/plan/ai-plan-card.tsx:750-751`) optimiza desde la página, sin tarjeta de aprobación. Todo test que afirme "no gastó" afirma los dos: ningún approval aprobado Y cero `POST /api/ai/plan {action:"optimize"}`. Afirmar uno solo certifica media compuerta.

Armalo en un solo test con `test.step`:

1. Camino del ticket, rechazado. Turno mockeado que emite un evento `approval` (`{ type: "approval", approval: { approvalRequestId, preflight } }`) y un `done` con `pendingApprovalIds: [approvalRequestId]`. Copiá la forma exacta de `e2e/ai-planner.spec.ts:118-228` (test AI2) y los campos de `AiApprovalPreflight` de `lib/ai/events.ts:254-272`. Esperá `ai-approval`, click en `ai-approval-decline`. Evidencia: el último body trae `approvals: [{ approvalRequestId, approve: false }]` y aparece `ai-approval-decided`.
2. Camino de la página, sin usar. En la misma conversación (otro mensaje) o en una página nueva si conviven mal: `operation` con `depot.ref` y `fleet.ref` a filas del catálogo (como en la tarea de R2), `attach`, llegar a `ai-plan-card[data-phase="confirming"]` con el `ai-plan-run` (scope dentro de `ai-plan-review`) habilitado, y NO apretarlo. Evidencia: `expectNoSpend({ bodies, planCalls })` pasa y `ai-plan-launched` no existe.
3. Control positivo: ahora sí apretá run, con `optimize` mockeado devolviendo una tarjeta `queued` y el resto de las rutas que necesita una corrida (copiá `e2e/ai-roadmap.spec.ts:401-415`: `queued`, `**/api/optimization/plans`, `**/api/ai/runs/**`, la geometría). Evidencia: en `planCalls` hay exactamente un `optimize`. Esto prueba que el detector ve un gasto cuando lo hay; un "no gastó" que no puede ver un gasto no vale nada.

Si el ticket y el roadmap no conviven en una conversación mockeada, separalos en dos páginas dentro del mismo test y decilo en el handoff.
...
3. Corré `npx playwright test e2e/ai-roadmap-invariants.spec.ts -g "R8-two-charge-paths" --repeat-each 3`: los 3 verdes. Después corré el archivo entero.
