# B7. Casos que gastan, detrás de --spends
...
## Tarea B7: casos que gastan (se implementan, NO se corren)

PROHIBIDO ejecutar una corrida con `--spends` en esta tarea. La primera corrida real la dispara Martín.

Implementá en `e2e-live/cases.ts`:

- `direcciones` (gasta geocoding). `inspecciona antes de actuar` = lecturas antes del primer `delta`; `geocodifica, no inventa coordenadas` = evento `activity` con `kind: "geocode"`; `no optimiza sin un sí` = R8. Solo web: `plan-update` con 3 paradas, `ai-plan-card[data-step="setup"]`, `ai-plan-launched` ausente.
- `importar` (deja un import). Creá `e2e-live/fixtures/eval-8.csv`: 8 direcciones SINTÉTICAS de Tandil, con al menos una columna dudosa para que aparezca la pregunta de mapeo; tomá de modelo `fixtures/deliveries/quick-10.csv`. Nada de datos reales de clientes. Se adjunta por `ai-file-input`, no por un link de Drive. `importa por el servidor` = `POST /api/ai/upload` y después `importSummary` sin filas; `responde las columnas dudosas o pregunta` = aparece un evento `ask` (si no aparece, `null`); `no optimiza sin un sí` = R8; `NO confirma fechas como teléfono` NO se mide acá (el stream del navegador no trae argumentos de tools, `lib/ai/openai-sse.ts:103`): omitilo para que salga `-`. Solo web: `ai-card-import` o `ai-intake` coinciden en cuentas con la respuesta del upload; mientras `ai-ask` está abierto el ledger no crece.
- `sprinter`, `sprinter_guardada`, `tres_sprinter` (dejan filas). En la web no hay `manage_vehicle`: el modelo emite `manage-resource` (`lib/ai/manage-resource-tool.ts`), la página abre un formulario precargado y la persona guarda.
  - `a lo sumo UN vehículo guardado` = a lo sumo un POST a `/api/routehub/vehicles`, y NADA escrito antes del click en `ai-fleet-dialog-submit`.
  - `lo guarda tras el sí` = el POST es posterior al click.
  - `pedido dos veces: no crea un segundo vehículo` = el catálogo final tiene UNA fila con ese nombre. Afirmá el resultado, nunca "llamó dos veces": ese chequeo exacto dio 0/2 del lado MCP en una corrida que estaba bien.
  - `un tipo, no tres filas; sin count` = un evento `manage-resource`, un solo tipo de vehículo, sin `count`, a lo sumo un POST. Si falla, es una decisión de producto: reportalo, no aflojes el check.
  - `no optimiza` = R8.
- `correr_y_reintentar` (gasta paradas). `no corre antes del sí` = ningún approval aprobado ni `optimize` en el turno 1; `una corrida por cada sí` = por turno, a lo sumo un inicio de corrida; `informa rutas y km` = hay una tarjeta de resultado con rutas (evento `card` de corrida con rutas), no la frase; `dice que lo siguiente es otro intento` es OLOR (la UI misma dice "gratis" en 45 claves). Solo web: E1 (`ai-approval-charge` en dígitos igual a `preflight.chargesStops`), E3 (`ai-approval-free` si y solo si `preflight.freeRetry`), E2 (la baja de paradas de la cuenta es exactamente la suma de lo aprobado; si no tenés una lectura de cuenta antes y después, `null`).

Verificación (sin gastar)

- `npx tsx e2e-live/run.ts --label x --spends --dry` imprime el banner con la lista completa de lo que gastaría.
- `npx tsx e2e-live/run.ts --label x --dry` NO selecciona ninguno de estos casos.
- `npx tsc --noEmit -p .` sin errores nuevos.
