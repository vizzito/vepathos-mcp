# B1. Esqueleto del arnés online: nombres, CLI y banner (sin red)
...
## Tarea B1: esqueleto, nombres y banner (sin red, sin navegador)

Leé antes: `/Users/martinvizzolini/workspace/vepathos-mcp/devtools/agent_eval.py` ENTERO (674 líneas). Ya resolvió los problemas que vas a tener: evidencia contra prosa, acentos, por turno contra agregado, "no medido" distinto de "falló". Anclas: `Case` `:288-295`, `CASES` `:300-435`, `COMMON` `:271-276`, esquema de salida `:550-599`, `list_battery` `:602-611`, `compare()` `:614-643`. Y `scripts/prompt-live.ts` de este repo, para copiar su estilo de script.

Creá (todo nuevo, bajo `e2e-live/`):

1. `names.ts`: por cada caso, clave, nivel, turnos, `spends` y nombres de chequeo, copiados BYTE A BYTE de `agent_eval.py` (acentos, mayúsculas, paréntesis, `m³` con U+00B3). Turnos: los de `agent_eval.py` sin el prefijo `vepathos, `, salvo donde la doc §4 da otra frase literal. Las 14 claves: `cuenta`, `capacidades`, `flota`, `direcciones`, `importar`, `inyeccion`, `25_vehiculos`, `volumen`, `duracion`, `correr_y_reintentar`, `sprinter`, `sprinter_guardada`, `tres_sprinter`, `sin_si`. También los 4 nombres de `COMMON`.
2. `cases.ts`: `type Case = { level: number; turns: string[]; checks: Record<string, Check>; smells: Record<string, Smell>; spends: string; drive?: Drive }`, espejo de `Case(level, turns, checks, spends)`. Un `Check` recibe lo observado en la corrida y devuelve `true | false | null`; `null` es "no se pudo medir". Un `Smell` lee prosa y nunca es rojo. Por ahora los casos van sin checks implementados (eso es B4 a B7); sí con `level`, `turns` y `spends`.
3. `run.ts`: CLI con `--label` (obligatorio para correr), `--runs` (default 3), `--case` (repetible), `--spends`, `--yes`, `--base-url`, `--list`, `--dry`. Carga de env copiando el lazo de `scripts/prompt-live.ts:48-60`.

Reglas de gasto (Martín pidió cuidado explícito con esto)

- Grupo que no gasta: `cuenta`, `capacidades`, `flota`, `25_vehiculos`, `volumen`, `duracion`, `sin_si`, `inyeccion`. Gastan: `direcciones` (geocoding), `sprinter`, `sprinter_guardada`, `tres_sprinter` (dejan filas), `importar` (deja un import), `correr_y_reintentar` (paradas del plan).
- Sin `--spends` solo se seleccionan los que no gastan. Un `--case` que gasta sin `--spends` es un error con mensaje claro, no un silencio.
- `--list` imprime la batería por nivel con lo que gasta cada caso y no toca la red.
- Antes de correr, imprimí: modelo (`openaiAiModel()` leído de `lib/ai/config.ts`, no asumido), esfuerzo de razonamiento, base URL, host de `APIDOC_URL` y de `VEPATHOS_MCP_DIRECT_URL` y `VEPATHOS_MCP_HTTP_URL` (SOLO hosts, jamás valores de claves), usuario con el que se va a loguear, casos x corridas x turnos, costo estimado (copiá el idiom de `scripts/prompt-live.ts:657-660`), la lista de lo que se va a gastar, y este aviso: acuñar la credencial puede revocar y recrear la API key "Vepathos AI Planner" de esa cuenta.
- Después pedí confirmación por stdin. Sin TTY y sin `--yes`, negate a correr. `--dry` imprime todo lo anterior y sale sin abrir navegador.
- Con `--spends` y un host MCP de producción (`mcp.vepathos.com`), negate siempre, como `agent_eval.py:479-498`.

4. Chequeo de sincronía: una función (corre en `--list` y al empezar) que lee `agent_eval.py` como texto y falla con un mensaje claro si alguna clave de caso o nombre de chequeo de `names.ts` no aparece literal en ese archivo.

Verificación

- `npx tsx e2e-live/run.ts --list` y `npx tsx e2e-live/run.ts --label x --dry` funcionan sin red y sin navegador.
- `echo | npx tsx e2e-live/run.ts --label x` se niega (no hay TTY ni `--yes`).
- `npx tsx e2e-live/run.ts --label x --case sprinter --dry` da error porque falta `--spends`.
- `npx playwright test --list` no muestra nada de `e2e-live/`.
- Ningún valor de clave en la salida.
