...
## Tarea A11: cierre de la Parte A (no escribe tests nuevos)

Pasos

1. `npx playwright test e2e/ai-roadmap-invariants.spec.ts`.
2. `npx playwright test e2e/ai-roadmap.spec.ts e2e/ai-planner.spec.ts` y compará test por test con `A0.md`. Un rojo que ya estaba en A0 se anota y no se arregla. Un rojo nuevo se investiga: lo único que tocó la Parte A en archivos existentes es `e2e/ai-roadmap.spec.ts` (A1).
3. Revisá el spec nuevo contra esta lista y corregí solo lo que falte:
   - 8 tests nombrados por la regla que prueban, cada uno con el `archivo:línea` de su regla en un comentario.
   - Ningún `toEqual([...])` de secuencia sobre `written.map(...)`: el orden solo se afirma con `before(...)`.
   - Ninguna aserción sobre el texto de `ai-message-assistant`.
   - Todo "no gastó" pasa por `expectNoSpend` (los dos caminos).
   - Ningún volcado de debug, `page.pause()` ni `test.only`.
4. Handoff de cierre: una tabla R1..R8 con estado (verde, o `fixme` con su motivo), qué evidencia sostiene cada uno, qué quedó sin poder observarse desde la UI y dónde se mide, y `git status --short -- e2e`. Confirmá que `git log -1 --format=%H` es el mismo hash que en `A0.md`.
