...
Regla: mientras un efecto está afuera, el motor no se consulta, "so two creates can never race and a step cannot be started twice" (`components/ai/roadmap/use-roadmap-executor.ts:12-14`; ref `flight` en `:169-170`; guarda al principio de `pump` en `:200`).

Estado a construir (la ventana de carrera): `goal: "resources"`, un vehículo con `count: 2`, `keepVehicles: true`, flota nueva con nombre, y un chofer que maneja ese vehículo. Cuando el vehículo cierra, `fleet.create` y `driver.create` quedan listos A LA VEZ (los dos dependen solo de `policy` y `vehicle.create`). Creates demorados a propósito: `mockCreates(page, catalog, { delayMs: 1500 })`. `test.setTimeout(90_000)`.

Evidencia

- Principal (ledger): esperá los tres `ai-saved-*`. Para todo par `i < j`, `written[i].doneAt <= written[j].at`: ningún create llegó mientras otro seguía sin responder. Y cada `what` aparece exactamente una vez (un step no arranca dos veces).
- Secundaria (DOM): `watchDom` instalado antes de `say`; `seen.maxRunning <= 1`.
- En el handoff decí qué `maxRunning` observaste. Si fue 0, la evidencia DOM es vacía (las filas `running` no se renderizan en ese recorrido): decilo con esas palabras y dejá la del ledger como la que sostiene el test.
...
3. Corré `npx playwright test e2e/ai-roadmap-invariants.spec.ts -g "R3-one-in-flight" --repeat-each 3`: los 3 verdes. Después corré el archivo entero.
