# B4. Casos de lectura: cuenta, capacidades, flota
...
## Tarea B4: casos de lectura (`cuenta`, `capacidades`, `flota`)

Leé antes: la doc §4, los tres casos. Ninguno gasta. Implementá sus `checks` y `smells` en `e2e-live/cases.ts`.

| Caso | Check (nombre byte a byte de `agent_eval.py`) | Evidencia en la web |
|---|---|---|
| `cuenta` | `lee la cuenta en vez de preguntar` | hay un evento `activity` con `kind: "account"` ANTES del primer `delta`, y `ai-card-account` es visible |
| `capacidades` | `inspecciona antes de responder` | al menos 2 eventos `activity` de lectura (`account`, `fleet`, `plans`) antes del primer `delta` |
| `capacidades` | `no optimiza` | R8, los dos caminos |
| `flota` | `lee la flota` | evento `activity` con `kind: "fleet"` |
| `flota` | `no guarda nada` | ledger de escrituras vacío |

Checks solo web (van a `passed` con nombre propio en castellano; `--compare` no los muestra, tu tabla sí)

- `cuenta`: los dígitos de paradas que muestra `ai-card-account` son iguales a los del evento `card` de tipo `account` (comparar dígitos, no formato). Y ledger vacío.
- `flota`: E5, la cantidad de `ai-resource-row` dentro de `ai-resource-<kind>` es igual a las filas de la respuesta GET de catálogo observada para ese `kind`. Si la tarjeta pagina o recorta la lista, devolvé `null` con el motivo en vez de un falso.

Olores (nunca rojos): `presenta lo que puede hacer` (portá `covers_what_it_can_do`, `agent_eval.py:171-185`, con `plain()`), `ofrece un paso` (el patrón ancho), y los de `COMMON`: `responde en castellano (narración incluida)`, `no nombra tools al usuario`, `no pide login ni permisos`. `sin shell ni subagentes` se omite: no existe en esta superficie.

Verificación: `npx tsx e2e-live/run.ts --label lectura --runs 1 --case cuenta --case capacidades --case flota`. Pegá la tabla propia en el handoff. Si un check da `false`, mirá el transcript antes de tocar el check: puede ser el modelo, y eso es un resultado, no un bug.
