# A9. Test R7: la compuerta del resumen del plan
...
Regla: `canShowPlanSummary` (`lib/ai/conversation.ts:75-87`) esconde `ai-plan-review` (`components/ai/plan/ai-plan-card.tsx:727`) y corta tanto `checkPlan` (`components/ai/use-ai-chat.ts:1623`) como `runPlan` (`:1815`). Es el invariante que más peso carga en la UI.

Corrección al prompt original: NO hay ~20 aserciones de `ai-planner.spec.ts` sobre `ai-plan-review` (aparece 0 veces ahí). La dependencia es indirecta: los tests que aprietan `ai-plan-run` (AI10 `:621`, AI12 `:730`, AI18 `:996-1009`, AI26 `:1370`, AI28 `:1476-1501`) pasan por esta compuerta. Este spec no toca código de producto, así que si algo de esa suite sale rojo, corré `npx playwright test e2e/ai-planner.spec.ts` sola y comparalo con `A0.md` antes de sospechar de tu test.

Estado a construir: el de `e2e/ai-roadmap.spec.ts:489-498` con `mockOrdersAndCost`, más el PATCH de la flota mockeado copiando `e2e/ai-roadmap.spec.ts:500-508`.

Evidencia, una compuerta por vez (en cada una `await expect(page.getByTestId("ai-plan-review")).toHaveCount(0)`):

1. Antes de los pedidos: faltan campos (`conversation.ts:78`).
2. Tras `attach`, con `cost:infeasible` abierta: la flota no alcanza y no se aceptó parcial (`:82-83`), y hay un step `asking` (`:86`). `planCalls` vacío.
3. Tras click en `ai-roadmap-option-adjust`: `cost` estacionado en el ajuste para esta revisión (`:81`). También el `ai-plan-run` con scope tiene `count` 0. Esperá 650 ms (como `e2e/ai-roadmap.spec.ts:516`) y `planCalls` sigue vacío.
4. Resolvé como `e2e/ai-roadmap.spec.ts:515-521`: en `ai-fleet-dialog` poné 3 unidades y `ai-fleet-dialog-submit`. La revisión cambia, llega un preflight, y AHORA `ai-plan-review` es visible y su `ai-plan-run` está habilitado.
5. `expectNoSpend` al final: nunca se apretó run.
...
3. Corré `npx playwright test e2e/ai-roadmap-invariants.spec.ts -g "R7-summary-gate" --repeat-each 3`: los 3 verdes. Después corré el archivo entero.
