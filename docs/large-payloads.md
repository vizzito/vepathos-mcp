...
| Dataset TTL | 24 h (product rule; not a configured env var in this adapter) | The plan the stops loaded into stays (library rules). |
...
MCP runs are plans ([architecture-mcp-plans.md](architecture-mcp-plans.md)): one import loads one
plan; import into the same `plan_id` to replace its stops.
