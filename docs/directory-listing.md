...
Tools: get_account, list_fleet, list_plans, list_datasets and list_automations (the connected account); import_deliveries, get_import_result and update_import_mapping (a spreadsheet, a link or pasted rows become a plan, without the rows passing through the chat); geocode_addresses and get_geocode_result (street addresses → coordinates via Vepathos Smart Import); manage_catalog (save a vehicle or a depot in the account); optimize_routes and get_optimization_result (async VRP for a saved plan, an import or stops with coordinates; poll with optimization_id); create_optimization_map (a 48-hour public link to the result); create_automation (prepares a rule that routes on a schedule, switched off — only the user turns one on). Optimize does not invent coordinates — confirm unresolved or low-confidence pins before routing. A rerun of the same plan within 24 hours, with the same stops or fewer, charges nothing.
