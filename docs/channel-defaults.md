...
| `optimize_time_windows` | lot `schedule.respect_delivery_windows` | plan capability + windows + shop flag | `options` + plan capability | `use_time_windows` (Core `constraints.time_windows`); omitted: on when a stop has a window |
...
| `rebalance_by_volume` / `by_weight`, load ratios | lot `rebalance.*` > model 0.4–0.95 / 0.01–0.95 | shop flags, `runDefaults.loadWeight/loadVolume` (0–0.95) | `options.*` > 0–1 | `use_volume` / `use_weight` (Core `constraints`); omitted: on when a vehicle declares that capacity. Ratios 0–1. Until 2026-09-17 always off, so capacities never bound |
...
| `min_stops` / `max_stops` (`deliveries_qty`) | vehicle overrides > defaults > lot route stops > computed fallback | `VehiclePreset` | request; `min` omitted when not sent | request; `min` omitted when not sent |
| Stop band (all channels) | **Engine** (`route-optimizer-app` `src/helpers/stop_band.py`), on every request: min omitted → 50% of max; min ≤ floor(max × 0.9) (lowered, `stop_band_margin`); Σ(qty × min) ≤ ceil(stops × 0.95) (lowered, `fleet_min_above_stops`). RouteHub also rejects lot totals outside ±5% of the addresses when queuing. Mirrors for previews: `lot_plan_adapter.py` log, Core `resolveStopBand`, MCP `preflight_checks.py` | same | same | same |
